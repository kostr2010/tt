...
int _TreeGlueSubtree(Tree* tree, Node* subtree, int* index, int nodesCount, int node, int branch) {    
    if (TreeInsertNode(tree, node, branch, subtree[*index].data) == -1) {
        printf("  [_TreeGlueSubtree] can't insert node's %d %d's branch!\n", node, branch);
        return -1;
    }

    int this = tree->nodes[node].branch[branch];

    printf("[_TreeGlueSubtree] glued <%s> to addr %d \n"
           "                   as %d branch of <%s> node of original tree\n"
           "                   with addr: %d\n", subtree[*index].data, this, branch, tree->nodes[node].data, node);
